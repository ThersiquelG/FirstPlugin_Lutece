...
Créez le répertoire   ``<lutece-dev>/lutece-dev-plugin-example/src/conf/default/WEB-INF/conf/`` et collez le fichier `db.properties` dedans.
(Vous pouvez également utiliser le répertoire ``/home/<user>/lutece/conf/plugin-example/WEB-INF/conf/`` afin d'éviter de publier votre configuration locale de développement dans le projet.)
